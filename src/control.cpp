...
Control::Control(std::unique_ptr<SettleUtil> settleUtil) : m_settleUtil(std::move(settleUtil)) {}
...
TimeSettler::TimeSettler(float smallError, uint32_t smallErrorTime, float largeError, uint32_t largeErrorTime,
                         uint32_t maxTime)
    : m_smallError(smallError), m_smallErrorTime(smallErrorTime), m_largeError(largeError),
      m_largeErrorTime(largeErrorTime), m_maxTime(maxTime) {}
...
PID::PID(float kp, float ki, float kd, float iMax, std::unique_ptr<SettleUtil> settleUtil)
    : Control(std::move(settleUtil)), m_kp(kp), m_ki(ki), m_kd(kd), m_iMax(iMax) {}
